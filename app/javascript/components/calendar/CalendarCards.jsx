...
function CalendarCards({movie,projection}) {
  ...
  useEffect(() => {
    ...
    setMovieCard(movieCards => [...movieCards,
      <Link to={"/movies/"+ movie.movie.slug} key={movie.movie.slug+projection.projection.id} className={"text-center text-sm border mb-1 border-black h-46 w-36 bg-" + movie.include.cycle.color}>
        <div className=''>

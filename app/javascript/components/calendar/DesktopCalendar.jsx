...
function DesktopCalendar({movies}) {
  ...
  useEffect(() => {
    ...
    if(monthIndex > 0) {
      for (let i = 0; i < daysOfPreviousMonth; i++) {
        ...
        setCalendarGrid(calendarGrid => [...calendarGrid,<div className='w-full h-full flex justify-center pb-2 bg-white' id={i+"prevMonth"} key={i+" prevMonth"}><div className='text-gray-300 h-fit flex flex-col items-center bg-black w-fit px-2' >{dayHeader} {calendarHelperInfo[monthIndex-1].length-daysOfPreviousMonth+i+1}</div></div>])
      }
    } else {
      for (let i = 0; i < daysOfPreviousMonth; i++) {
        const dayHeader = Object.keys(firstDayOfMonthHelper)[i].charAt(0).toUpperCase() + Object.keys(firstDayOfMonthHelper)[i].slice(1)
        setCalendarGrid(calendarGrid => [...calendarGrid,<div className='w-full h-full flex justify-center pb-2 bg-white' id={i+"prevMonth"} key={i+" prevMonth"}><div className='text-gray-300 h-fit flex flex-col items-center bg-black w-fit px-2' >{dayHeader} {calendarHelperInfo.decemberYearBefore.length-daysOfPreviousMonth+i+1}</div></div>])
      }
    }
    ...
    for (let i = 0; i < daysOfCurrentMonth; i++) {
      ...
      <div className='w-full h-full flex flex-col bg-white 'id={i+"currentMonth"} key={i+" currentMonth"}>
    ...
    if(monthIndex < 11) {
      for (let i = 0; i < daysOfNextMonth; i++) {
        ...
        setCalendarGrid(calendarGrid => [...calendarGrid,<div className='w-full h-full flex justify-center pb-2 bg-white' key={i+" nextMonth"}><div className='text-gray-300 h-fit flex flex-col items-center bg-black w-fit px-2' id={i+"nextMonth"} >{dayHeader} {i+1}</div></div>])
      }
    } else {
      for (let i = 0; i < daysOfNextMonth; i++) {
  ...
  return (
    <div className='mb-5'>
      ...
      <div className='grid gap-2 grid-cols-7 justify-center text-sm w-[100vw] lg:w-[90vw] mx-auto bg-gray-200'>
        {calendarGrid}
      </div>
    </div>
  )

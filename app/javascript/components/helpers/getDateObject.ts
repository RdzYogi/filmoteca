...
function getDateObject(dateString: string,
  ...
  if (dayLong) {
    const intlDate = new Intl.DateTimeFormat('es-ES',{year: "numeric",month: optionMonth, weekday: "long",hour:"2-digit",minute: "2-digit"}).format(date)
    ...
    dateObject.month = monthLong ? intlDate.split(" ")[2] : intlDate.split("/")[1]
    dateObject.year = monthLong ? intlDate.split(" ")[4] : intlDate.split("/")[1]
